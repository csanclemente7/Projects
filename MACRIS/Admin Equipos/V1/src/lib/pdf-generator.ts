...
export async function generateReportPDF(
    ...
    // Comprobación más flexible para el tipo de servicio
    const isInstallation = report.serviceType.toLowerCase().includes('montaje') || report.serviceType.toLowerCase().includes('instalaci');
    ...
    if (isInstallation && report.itemsSnapshot && report.itemsSnapshot.length > 0) {
    ...
    if (isInstallation && (report.photo_internal_unit_url || report.photo_external_unit_url)) {
        ...
        const addPhotoToDoc = (label: string, dataUrl: string | undefined | null, x: number, y: number): number => {
            ...
            try {
                ...
                doc.addImage(dataUrl, 'AUTO', x, y + 4, photoMaxWidth, height);
                return height + 4; 
            } catch (e: any) {
    ...
    if (report.clientSignature && report.clientSignature.startsWith('data:image')) {
        try {
            ...
            doc.addImage(report.clientSignature, 'AUTO', sigX, sigY, sigWidth, sigHeight);
        } catch (e: any) {
