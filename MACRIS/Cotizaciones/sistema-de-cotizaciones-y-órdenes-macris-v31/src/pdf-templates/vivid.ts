...
// --- Template 4: Vivid ---
export function renderVividPDF(doc: jsPDF, quote: Quote, client: Client | undefined, logoUrl: string) {
    const primaryColor = '#00A8C5';
    const margin = 40;
    ...
    addHeaderQuoteInfo(doc, quote, client, pageWidth - margin, y + 10, '#FFFFFF');
    
    y = 130;

    // Client Info
    y = addServiceLocationClientBlock(doc, quote, client, y, primaryColor, 0);
    y += 10;
    ...
    doc.setFillColor('#F0F9FB');
    doc.rect(totalsX - 10, totalsY + 5, pageWidth - margin - totalsX + 20, 25, 'F');
    totalsY += 20;
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
