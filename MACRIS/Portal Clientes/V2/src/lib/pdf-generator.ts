...
import { normalizeSignatureImage } from './signature-utils';
...
function resolveReportIdValue(report: Report, allOrders: Order[]): string {
...
export function buildReportPdfFilename(report: Report, allOrders: Order[]): string {
    const idValue = resolveReportIdValue(report, allOrders);
    ...
    return `Reporte_${clientName?.replace(/\s/g, '_') || 'General'}_${filenameId}.pdf`;
}
...
async function renderReportToDoc({
...
}: ReportRenderContext): Promise<void> {
    if (startOnNewPage) {
        doc.addPage();
    }
    ...
    try {
        ...
        if (resolvedLogo === undefined) {
            resolvedLogo = await getLocalImageAsDataUrl('MacrisLogo.png');
        }
        if (resolvedLogo) {
            const aspectRatio = 1823 / 1440;
            const logoHeight = 25;
            const logoWidth = logoHeight * aspectRatio;
            doc.addImage(resolvedLogo, 'PNG', margin, currentY, logoWidth, logoHeight);
        }
    } catch (e) {
    ...
    if (report.clientSignature && report.clientSignature !== "PENDING_SIGNATURE") {
        try {
            const signatureDataUrl = await normalizeSignatureImage(report.clientSignature);
...
export async function generateReportPDF(
    ...
    const filename = buildReportPdfFilename(report, allOrders);
...
export async function generateReportsBundlePDF(
    reports: Report[],
    cities: City[],
    companies: Company[],
    dependencies: Dependency[],
