...
import { formatDate, formatTime, resizeCanvas } from './utils';
...
export async function openReportFormModal(options: { report?: Report; equipment?: Partial<Equipment>; category?: 'empresa' | 'residencial', isFromOrder?: boolean; serviceType?: string; order?: Order } = {}) {
    ...
    if (report) {
        ...
        if (selectedCategory === 'residencial') {
            ...
            if (companyIdToSelect) {
                D.reportCompanySelect.value = companyIdToSelect;
        ...
        if (selectedCategory === 'empresa') {
            ...
            if (matchedCompany) {
                D.reportCompanySelect.value = matchedCompany.id;
                // This populates dependencies and sets city from company record.
                updateLocationDropdownsFromCompany(matchedCompany.id);
            }
            // Now, override the city with the one from the order, if it was found/created.
            if (city) {
    ...
    if (report?.photo_internal_unit_url && report.photo_internal_unit_url !== 'PENDING_PHOTO') {
        ...
        D.photoInternalUnitPreview.style.display = 'block';
    ...
    if (report?.photo_external_unit_url && report.photo_external_unit_url !== 'PENDING_PHOTO') {
        ...
        D.photoExternalUnitPreview.style.display = 'block';
...
async function saveSignature() {
    ...
    const updateData = { clientSignature: dataUrl };
    ...
    try {
        ...
        // --- TRY ONLINE ---
        await updateMaintenanceReport(reportId, dbUpdateData);
        
        // --- ONLINE SUCCESS ---
        hideLoader();
        const reports = State.currentUser.role === 'admin' 
            ? await fetchAllReports() 
            : await fetchReportsForWorker(State.currentUser.id);
        ...
        if (isNetworkError) {
            ...
            try {
                ...
                if(reportInState) reportInState.clientSignature = dataUrl;
...
export function openPhotoCaptureModal(type: 'internal' | 'external') {
    ...
    D.photoCaptureFeedback.textContent = 'Apuntando a la cámara...';
    
    navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
        .then(stream => {
            photoCaptureStream = stream;
            D.photoCaptureVideo.srcObject = stream;
            D.photoCaptureVideo.setAttribute("playsinline", "true");
            D.photoCaptureVideo.play();
...
export async function handlePhotoCaptured() {
    if (!D.photoCaptureVideo || D.photoCaptureVideo.readyState !== D.photoCaptureVideo.HAVE_ENOUGH_DATA || !State.currentUser) {
        showAppNotification('El video de la cámara no está listo.', 'warning');
        return;
    }
    ...
    let width = D.photoCaptureVideo.videoWidth;
    let height = D.photoCaptureVideo.videoHeight;
    if (width > height) {
    ...
    ctx.drawImage(D.photoCaptureVideo, 0, 0, width, height);
    const dataUrl = canvas.toDataURL('image/jpeg', 0.8);

    closePhotoCaptureModal(); // Close modal immediately

    // --- LOGIC FOR EDITING AN EXISTING REPORT ---
    if (State.contextForPhotoUpdate) {
        const { reportId, photoType } = State.contextForPhotoUpdate;
        
        const updateData: Partial<Report> = {};
        const dbUpdateData: any = {};
        if (photoType === 'internal') {
            updateData.photo_internal_unit_url = dataUrl;
            dbUpdateData.photo_internal_unit_url = dataUrl;
        } else {
            updateData.photo_external_unit_url = dataUrl;
            dbUpdateData.photo_external_unit_url = dataUrl;
        }
        
        showLoader("Actualizando foto...");
        try {
            if (!navigator.onLine) throw new Error('Offline');
            ...
            const reports = State.currentUser.role === 'admin' 
                ...
                : await fetchReportsForWorker(State.currentUser.id);
            State.setReports(reports);
            showAppNotification('Foto actualizada con éxito.', 'success');
            openViewReportDetailsModal(reportId);

        } catch (err: any) {
            const isNetworkError = err.message === 'Offline' || err.message.includes('Failed to fetch');
            if (isNetworkError) {
                console.warn(`Online photo update failed for report ${reportId}. Falling back.`, err);
                try {
                    // --- OFFLINE LOGIC ---
                    await updateLocalReport(reportId, updateData);
                    
                    const reportInState = State.reports.find(r => r.id === reportId);
                    if (reportInState) { Object.assign(reportInState, updateData); }

                    showAppNotification('Foto actualizada localmente.', 'info');
                    openViewReportDetailsModal(reportId);
                } catch (localErr: any) {
                    showAppNotification(`Error al actualizar foto local: ${localErr.message}`, 'error');
                } finally {
                    hideLoader();
                }
            } else {
                // --- OTHER ERROR ---
                hideLoader();
                showAppNotification(`Error al actualizar la foto: ${err.message}`, 'error');
            }
        } finally {
...
export function openViewReportDetailsModal(reportId: string) {
...
D.downloadReportPdfButton.onclick = async () => {
    ...
    try {
        const pdfOutput = await generateReportPDF(report, State.cities, State.companies, State.dependencies, formatDate, State.allServiceOrders, 'open');

        if (typeof pdfOutput === 'string') {
if (Capacitor.isNativePlatform()) {
const webviewUrl = Capacitor.convertFileSrc(pdfOutput);
const viewerUrl = `https://docs.google.com/gview?embedded=true&url=${encodeURIComponent(webviewUrl)}`;
const newWindow = window.open(viewerUrl, '_blank');
if (!newWindow || newWindow.closed || typeof newWindow.closed === 'undefined') {
    showAppNotification('No se pudo abrir el visor de PDF dentro de la aplicación.', 'warning');
}
} else {
...
export function handleAssignmentCompanyChange() {
    ...
    const company = State.companies.find(c => c.id === newCompanyId);
    const originalReport = State.editLocationState.originalReport;

    if (!company) {
        D.editReportCityInput.value = '';
        D.editReportDependencySelect.innerHTML = '<option value="">Seleccione una empresa...</option>';
        D.editReportDependencySelect.disabled = true;
        return;
    }

    const city = State.cities.find(c => c.id === company.cityId);
    D.editReportCityInput.value = city ? city.name : 'N/A';

    const dependenciesForNewCompany = State.dependencies.filter(d => d.companyId === newCompanyId);
    populateDropdown(D.editReportDependencySelect, dependenciesForNewCompany);
    D.editReportDependencySelect.disabled = false;
    D.editReportDependencySelect.value = '';
    ...
    if (originalDependencyName) {
        const existingDependency = dependenciesForNewCompany.find(d => d.name.trim().toLowerCase() === originalDependencyName.trim().toLowerCase());
        if (existingDependency) {
            ...
            D.editReportDependencyWarning.innerHTML = `La dependencia "<b>${originalDependencyName}</b>" no existe para esta empresa. Se creará una nueva al guardar.<br>Si no desea crearla, seleccione una dependencia existente de la lista.`;
...
function getPaginatedData<T>(tableKey: MaintenanceTableKey, data: T[]): T[] {
    const { currentPage, itemsPerPage } = State.tablePaginationStates[tableKey];
...
export async function renderMyReportsTable() {
    ...
    if (!State.showAllMyReports) {
        ...
        userReports = userReports.filter(report => new Date(report.timestamp) >= twoDaysAgo);
    }
    ...
    // 4. Sort: pending reports first, then by date descending
    filteredReports.sort((a, b) => {
        const aIsUnsigned = !a.clientSignature || a.clientSignature === 'PENDING_SIGNATURE';
        const bIsUnsigned = !b.clientSignature || b.clientSignature === 'PENDING_SIGNATURE';
        const aHasPendingPhotos = a.serviceType === 'Montaje/Instalación' && (a.photo_internal_unit_url === 'PENDING_PHOTO' || a.photo_external_unit_url === 'PENDING_PHOTO');
        const bHasPendingPhotos = b.serviceType === 'Montaje/Instalación' && (b.photo_internal_unit_url === 'PENDING_PHOTO' || b.photo_external_unit_url === 'PENDING_PHOTO');
        
        const aIsPending = aIsUnsigned || aHasPendingPhotos;
        const bIsPending = bIsUnsigned || bHasPendingPhotos;

        if (aIsPending && !bIsPending) return -1;
        if (!aIsPending && bIsPending) return 1;
        
        // If both are same pending status, sort by date
    ...
    D.myReportsTableBody.innerHTML = paginatedReports.map(report => {
        ...
        const isUnsigned = !report.clientSignature || report.clientSignature === 'PENDING_SIGNATURE';
        const hasPendingPhotos = report.serviceType === 'Montaje/Instalación' && (report.photo_internal_unit_url === 'PENDING_PHOTO' || report.photo_external_unit_url === 'PENDING_PHOTO');
        const isPending = isUnsigned || hasPendingPhotos;
        ...
        const signatureButtonClass = isUnsigned ? 'btn-warning' : 'btn-accent';
        const signatureButtonText = isUnsigned ? 'Agregar Firma' : 'Gestionar Firma';
        ...
        return `
            <tr class="clickable-row ${rowClass}" data-report-id="${report.id}">
                ...
                <td data-label="Acciones">
                    <button class="action-btn view-report-btn btn-secondary" data-report-id="${report.id}" title="Ver Detalles"><i class="fas fa-eye"></i></button>
                    <button class="action-btn edit-signature-btn ${signatureButtonClass}" data-report-id="${report.id}" title="${signatureButtonText}"><i class="fas fa-signature"></i></button>
                </td>
            </tr>
        `;
    }).join('');
...
function getFilteredAdminReports(): Report[] {
    // Get filter values
    const searchTerm = State.tableSearchTerms.adminReports.toLowerCase();
...
export function renderAdminReportsTable() {
    ...
    D.adminReportsTableBody.innerHTML = paginatedReports.map(report => {
        ...
        const isUnsigned = !report.clientSignature || report.clientSignature === 'PENDING_SIGNATURE';
        const rowClass = isUnsigned ? 'report-unsigned' : '';

        // FIX: The `data-current-status` attribute requires a string, but was being passed a boolean (`report.is_paid`). This has been corrected by converting the boolean to a string.
        return `
             <tr class="${rowClass}">
                <td data-label="Fecha">${formatDate(report.timestamp)}</td>
...
export function openOrderDetailsModal(orderId: string) {
    ...
    D.orderClientPhone.textContent = order.clientDetails?.phone || 'N/A';
    D.orderClientEmail.textContent = order.clientDetails?.email || 'N/A';
