...
import { Report, User, City, Company, Sede, Dependency, Equipment, ServiceType, EquipmentType, RefrigerantType, AppSettings, Order, EntityType } from '../types';

const DB_NAME = 'maintenance-db';
const DB_VERSION = 5; // v5: added sedes store
    ...
    upgrade(db, oldVersion) {
      ...
      if (oldVersion < 5) {
        if (!db.objectStoreNames.contains('sedes')) {
          ...
          console.log('Object store "sedes" created.');
        }
      }
    },
  });
