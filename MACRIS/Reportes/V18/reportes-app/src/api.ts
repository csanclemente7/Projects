        ...
        switch (type) {
            ...
            case 'dependency':
                ...
                if (isClientDirect) {
                    ...
                    // Convención: client_id = empresa madre (= rawCompanyId, mismo UUID en clients),
                    //             company_id = registro raíz en maintenance_companies (mismo UUID),
                    //             sede_id = null (sin sede).
                    dependencyData.company_id = rawCompanyId;
                    dependencyData.client_id = rawCompanyId;
                    // sede_id queda null por defecto
                } else {
                    // Caso A: empresa CON sedes — rawCompanyId es el ID de la sede en maintenance_companies.
                    // Convención: company_id = sede (referencia técnica en maintenance_companies),
                    //             sede_id = sede (mismo valor, semántica explícita),
                    //             client_id = empresa madre real (companyId de la sede en State.sedes).
                    dependencyData.company_id = rawCompanyId;
                    dependencyData.sede_id = rawCompanyId;
                    const parentSede = State.sedes.find(s => s.id === rawCompanyId);
                    if (parentSede?.companyId) {
                        dependencyData.client_id = parentSede.companyId;
                    }
                }
