...
import {
    ...
    getCachedSetting,
    loadDesktopOrdersCache,
    loadDesktopQuotesCache,
    queueDesktopPendingMutation,
    setCachedSetting,
} from './db';
...
function isDesktopOfflineMode(): boolean {
    return isDesktopRuntime() && !navigator.onLine;
}

function shouldUseDesktopOfflineFallback(error: unknown): boolean {
...
function getHighestManualId(records: Array<{ manualId?: string | null }>, fallbackStartId: number): string {
    const maxId = records.reduce((highest, record) => {
        const current = parseInt(record.manualId || '', 10);
        return Number.isNaN(current) ? highest : Math.max(highest, current);
    }, fallbackStartId - 1);

    return String(Math.max(maxId + 1, fallbackStartId));
}

async function saveQuoteRemote(quote: Quote): Promise<Quote> {
...
async function saveOrderRemote(order: Order): Promise<Order> {
...
async function deleteQuoteRemote(quoteId: string): Promise<void> {
    await supabaseQuotes.from('quote_items').delete().eq('quoteId', quoteId);
    await supabaseQuotes.from('quotes').delete().eq('id', quoteId);
}

async function deleteOrderRemote(orderId: string): Promise<void> {
...
async function _findNextHighestManualId(
    ...
    if (isDesktopOfflineMode()) {
        if (tableName === 'quotes') {
            return getHighestManualId(loadDesktopQuotesCache(), fallbackStartId);
        }
...
export function getNextQuoteId(): Promise<string> {
    return (async () => {
        try {
            const remoteNext = await _findNextHighestManualId('quotes', 737);
            if (!isDesktopRuntime()) return remoteNext;
            const cachedNext = getHighestManualId(loadDesktopQuotesCache(), 737);
            return String(Math.max(parseInt(remoteNext, 10) || 737, parseInt(cachedNext, 10) || 737));
        } catch (error) {
            if (isDesktopRuntime()) {
                return getHighestManualId(loadDesktopQuotesCache(), 737);
            }
            throw error;
        }
    })();
...
export async function getNextOrderId(): Promise<string> {
    if (isDesktopOfflineMode()) {
        return getHighestManualId(loadDesktopOrdersCache(), 1001);
    }
    ...
    try {
        ...
        if (error || !data || data.length === 0) {
            ...
            if (isDesktopRuntime()) return getHighestManualId(loadDesktopOrdersCache(), 1001);
            return '1001';
        }
        const maxId = parseInt(data[0].manualId, 10);
        const remoteNext = isNaN(maxId) ? 1001 : maxId + 1;
        if (!isDesktopRuntime()) return String(remoteNext);
        const cachedNext = parseInt(getHighestManualId(loadDesktopOrdersCache(), 1001), 10);
        return String(Math.max(remoteNext, cachedNext));
    } catch (error) {
        if (isDesktopRuntime()) {
            return getHighestManualId(loadDesktopOrdersCache(), 1001);
        }
...
export async function saveQuote(quote: Quote): Promise<Quote> {
    ...
    try {
        const savedQuote = await saveQuoteRemote(quote);
        clearDesktopPendingMutation('quote', quote.id);
        return savedQuote;
    } catch (error) {
        if (!shouldUseDesktopOfflineFallback(error)) throw error;
        console.warn('Desktop mode: saving quote locally until remote sync is available.', error);
        const localQuote: Quote = {
            ...quote,
            items: [...(quote.items || [])],
            image_urls: [...(quote.image_urls || [])],
...
export async function saveOrder(order: Order): Promise<Order> {
    ...
    try {
        const finalOrder = await saveOrderRemote(order);
        clearDesktopPendingMutation('order', order.id);
        return finalOrder;
    } catch (error) {
        if (!shouldUseDesktopOfflineFallback(error)) throw error;
        console.warn('Desktop mode: saving order locally until remote sync is available.', error);
        const localOrder: Order = {
            ...order,
            items: [...order.items],
            technicianIds: [...order.technicianIds],
...
export async function deleteQuote(quoteId: string): Promise<void> {
    ...
    try {
        ...
        clearDesktopPendingMutation('quote', quoteId);
    } catch (error) {
        if (!shouldUseDesktopOfflineFallback(error)) throw error;
        console.warn('Desktop mode: deleting quote locally until remote sync is available.', error);
        queueDesktopPendingMutation({
            entity: 'quote',
            action: 'delete',
            id: quoteId,
...
export async function deleteOrder(orderId: string): Promise<void> {
    ...
    try {
        ...
        clearDesktopPendingMutation('order', orderId);
    } catch (error) {
        if (!shouldUseDesktopOfflineFallback(error)) throw error;
        console.warn('Desktop mode: deleting order locally until remote sync is available.', error);
        queueDesktopPendingMutation({
            entity: 'order',
            action: 'delete',
            id: orderId,
...
export async function syncQuoteToSupabase(quote: Quote): Promise<Quote> {
    return saveQuoteRemote(quote);
}

export async function syncOrderToSupabase(order: Order): Promise<Order> {
    return saveOrderRemote(order);
}

export async function removeQuoteFromSupabase(quoteId: string): Promise<void> {
    await deleteQuoteRemote(quoteId);
}

export async function removeOrderFromSupabase(orderId: string): Promise<void> {
    await deleteOrderRemote(orderId);
}
