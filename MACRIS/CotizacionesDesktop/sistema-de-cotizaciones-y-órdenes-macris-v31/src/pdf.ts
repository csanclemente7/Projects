...
function getCachedAssetDataUrl(assetUrl: string): Promise<string> {
    const existing = assetDataUrlCache.get(assetUrl);
    if (existing) return existing;
    const pending = getLogoAsDataUrl(assetUrl);
...
export async function generateOrderPDFDoc(order: Order): Promise<jsPDF> {
    ...
    const logoUrl = await getCachedAssetDataUrl(MacrisLogo); // Use standard logo for orders
...
async function appendImagesToPdf(doc: jsPDF, quote: Quote) {
    ...
    if (isDesktopRuntime() && !navigator.onLine) return;
    doc.addPage();
    doc.setFontSize(16);
...
async function dispatchPdfGeneration(
    ...
    const logoUrl = await getCachedAssetDataUrl(logoToUse);
