...
import {
    applyDesktopPendingMutations,
    initDesktopPersistence,
    loadDesktopCatalogSnapshot,
    loadDesktopOrdersCache,
    loadDesktopQuotesCache,
    saveDesktopCatalogSnapshot,
    saveDesktopOrdersCache,
    saveDesktopQuotesCache,
} from './db';
...
export function setQuotes(newQuotes: Quote[]) {
    quotes = newQuotes;
    if (isDesktopRuntime()) {
        saveDesktopQuotesCache(newQuotes);
    }
}
...
export function setOrders(newOrders: Order[]) {
    orders = newOrders;
    if (isDesktopRuntime()) {
        saveDesktopOrdersCache(newOrders);
    }
}
...
export type LoadStateResult = {
    usedCatalogCache: boolean;
    catalogCacheTimestamp: string | null;
    usedRecordCache: boolean;
    remoteDataUnavailable: boolean;
};
...
export async function loadState(): Promise<LoadStateResult> {
    ...
    const cachedCatalogs = desktopMode ? loadDesktopCatalogSnapshot() : null;
    const cachedQuotes = desktopMode ? loadDesktopQuotesCache() : [];
    const cachedOrders = desktopMode ? loadDesktopOrdersCache() : [];

    let usedCatalogCache = false;
    let catalogCacheTimestamp: string | null = null;
    let usedRecordCache = false;
    let remoteDataUnavailable = false;

    const [loadedQuoteAuthors, loadedOrderAuthors] = await Promise.all([
        fetchQuoteAuthors().catch(() => ({})),
        fetchOrderAuthors().catch(() => ({})),
    ]);
    ...
    try {
        const [remoteQuotes, remoteOrders] = await Promise.all([
            API.getQuotesFromSupabase(),
            API.getOrdersFromSupabase(),
        ]);
        loadedQuotes = desktopMode ? applyDesktopPendingMutations(remoteQuotes, 'quote') : remoteQuotes;
        loadedOrders = desktopMode ? applyDesktopPendingMutations(remoteOrders, 'order') : remoteOrders;
    } catch (error) {
        if (!desktopMode) throw error;
        console.warn('Desktop mode: remote quotes/orders unavailable, continuing with local record cache if available.', error);
        loadedQuotes = cachedQuotes;
        loadedOrders = cachedOrders;
        usedRecordCache = cachedQuotes.length > 0 || cachedOrders.length > 0;
        remoteDataUnavailable = true;
    }

    try {
        const [loadedItems, loadedClients, loadedTechnicians, loadedServiceTypes, loadedSedes, loadedDependencies] = await Promise.all([
        ...
        if (!desktopMode || !cachedCatalogs) throw error;

        console.warn('Desktop mode: loading catalogs from local cache.', error);
        usedCatalogCache = true;
        catalogCacheTimestamp = cachedCatalogs.savedAt;
        remoteDataUnavailable = true;

        setItems(cachedCatalogs.items);
        setClients(cachedCatalogs.clients);
        setTechnicians(cachedCatalogs.technicians);
        setServiceTypes(cachedCatalogs.serviceTypes);
        setSedes(cachedCatalogs.sedes);
        setDependencies(cachedCatalogs.dependencies);
    }
