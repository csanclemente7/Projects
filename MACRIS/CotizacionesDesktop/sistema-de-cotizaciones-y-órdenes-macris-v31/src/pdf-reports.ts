...
export async function getLocalImageAsDataUrl(url: string, format = 'image/jpeg'): Promise<string> {
...
function formatDate(dateInput?: Date | string | null, includeTime: boolean = true): string {
    if (!dateInput) return '';
    try {
...
export async function generateReportPDF(
    ...
    if (existingDoc && doc.getNumberOfPages() > 0) {
        doc.addPage();
    }
    ...
    // HEADER
    try {
        const logoDataUrl = await getLocalImageAsDataUrl(MacrisLogo, 'image/jpeg');
        if (logoDataUrl) {
    ...
    if (report.orderId && idValue !== report.id) {
        idValue = idValue;
    } else {
        idValue = report.id.substring(0, 8);
    }
