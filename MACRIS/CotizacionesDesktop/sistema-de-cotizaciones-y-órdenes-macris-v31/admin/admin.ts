...
async function handleSendRecoveryCode() {
    ...
    try {
        ...
        recoveryError.textContent = 'No se pudo generar el codigo.';
    } finally {
...
async function handleVerifyRecoveryCode() {
    ...
    const isExpired = Date.now() - payload.createdAt > RECOVERY_CODE_TTL_MS;
    if (isExpired) {
        clearRecoveryCode();
